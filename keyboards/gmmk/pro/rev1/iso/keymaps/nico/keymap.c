...
bool encoder_update_user(uint8_t index, bool clockwise) {
    ...
    if (mods_state & MOD_BIT(KC_LSFT)) { // If holding L shift, encoder changes screen brightness
        ...
        clockwise ? tap_code16(LGUI(LSFT(KC_Z))) : tap_code16(LGUI(KC_Z));
    } else if (mods_state & MOD_BIT(KC_LCTL)) { // If holding L Ctrl, encoder zooms in and out
        unregister_mods(MOD_BIT(KC_LCTL));
        clockwise ? tap_code16(LGUI(KC_EQL)) : tap_code16(LGUI(KC_MINS));
